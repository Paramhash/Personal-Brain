---
tags: ["api", "tastyworks", "margin", "trading", "financial-api", "pre-trade-validation"]
created: 2023-10-27
reviewed: false
source_origin: "margin-requirements.md"
---
# Tastyworks Margin Requirements API

The Tastyworks Margin Requirements API provides endpoints for retrieving current margin and capital requirements for an account, and for estimating the margin impact of a prospective order before submission. It is a critical component for pre-trade validation and risk management within the [[../entities/tastyworks.md|Tastyworks]] trading ecosystem.

The dry-run endpoint is particularly useful for pre-trade validation, allowing users to see how an order would affect buying power and margin requirements without actually placing it.

**Base URL:** `https://api.tastyworks.com`
**Authentication:** Requires a valid session token passed via the `Authorization` header.
**API Version:** 11.24.0

This API is part of the broader [[../concepts/api.md|API]] suite offered by Tastyworks, complementing functionalities like the [[../entities/tastyworks-order-management-api.md|Order Management API]]. It directly relates to [[../concepts/margin-trading.md|margin trading]] and capital management.

## Endpoints

### Get Account Margin Requirements

This endpoint allows fetching the current margin and capital requirements report for a specified account. It returns a detailed breakdown of margin requirements across all existing positions.
...
This endpoint is used to estimate the margin requirements and buying power effect of a prospective order without actually submitting it. It serves as the margin-specific equivalent of an order dry-run, focusing specifically on the capital impact rather than general order validation.
...
*   **This endpoint does not place an order.** It only evaluates the margin impact. Use the [[../entities/tastyworks-order-management-api.md|Order Management API]] to actually submit orders.