---
tags: [api, trading, finance, tastyworks, margin, risk]
created: 2023-10-27
reviewed: false
source_origin: "risk-parameters.md"
---
# Tastyworks Risk Parameters API

The [[../entities/tastyworks.md|Tastyworks]] Risk Parameters API provides endpoints for retrieving account [[../concepts/position-limits.md|position limits]], per-symbol [[../concepts/margin-requirements.md|margin requirements]], the global margin configuration (including the [[../concepts/risk-free-rate.md|risk-free rate]]), and raw [[../concepts/span-margin.md|SPAN margin data]] for futures exchanges.
...
Retrieve the publicly accessible, read-only global margin configuration. Currently returns the [[../concepts/risk-free-rate.md|risk-free rate]] used in margin calculations.
...
Retrieve raw [[../concepts/span-margin.md|SPAN (Standard Portfolio Analysis of Risk)]] margin data rows for a specific date and futures exchange. SPAN data is used by clearing firms to calculate futures and futures options margin requirements.
...
| `exchange` | query | string | Yes | The exchange: `[[../entities/cme-exchange.md|CME]]` or `[[../entities/cfe-exchange.md|CFE]]` |
...
| Field | Type | Description |
|-------|------|-------------|
| `risk-free-rate` | number (double) | The current [[../concepts/risk-free-rate.md|risk-free interest rate]] used in margin calculations (as a decimal, e.g., `0.0525` = 5.25%) |
...
| `exchange` | string | The exchange (`[[../entities/cme-exchange.md|CME]]` or `[[../entities/cfe-exchange.md|CFE]]`) |
| `row-data` | string | The raw SPAN data row content |

## Common Use Cases

*   **Pre-trade position limit checks:** Before submitting an order, check `GET /accounts/{account_number}/position-limit` to verify the order size doesn't exceed the account's [[../concepts/position-limits.md|limits]].
*   **Custom margin calculations:** Use `GET /accounts/{account_number}/margin-requirements/{underlying_symbol}/effective` to get the exact [[../concepts/margin-requirements.md|margin rates]] for a symbol, then calculate expected margin impact locally.
*   **Risk-free rate for pricing models:** The `GET /margin-requirements-public-configuration` endpoint provides the [[../concepts/risk-free-rate.md|risk-free rate]] used in margin calculations, which is also useful as an input for options pricing models (Black-Scholes).
*   **Futures margin analysis:** Use the [[../concepts/span-margin.md|SPAN rows]] endpoint to access raw SPAN data for advanced futures margin analysis and risk management.
