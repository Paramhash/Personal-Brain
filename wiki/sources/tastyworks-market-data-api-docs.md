---
tags: ["source", "api-documentation", "market-data"]
created: 2023-10-27
reviewed: false
source_origin: "market-data.md"
---
# Market Data API Documentation (Tastyworks)

This document provides a comprehensive overview of the [[Market Data API (Tastyworks)|Tastyworks Market Data API]], a [[REST API]] endpoint for fetching point-in-time market data snapshots. It serves as a non-streaming alternative to the [[DXLink]] WebSocket API for one-time quotes.

**Provider:** [[Tastyworks]]
**Base URL:** `https://api.tastyworks.com`
**Authentication:** Requires a valid session token passed via the `Authorization` header. See [[API Authentication]] for details.
**API Version:** v0
...
Fetch market data for multiple symbols, organized by [[Market Data Instrument Types|instrument type]]. The combined limit across all types is **100 symbols per request**. See [[API Rate Limits]] for more.
...
**Important:** The parameter names use **singular hyphenated** form (`equity-option`, not `equity-options`). This is a common source of errors. See [[API Naming Conventions]].

**Response** — `200 OK`

Returns an array of [[MarketData Object|`MarketData` objects]], one per requested symbol.
...
A comprehensive [market data snapshot](../concepts/market-data-api.md) for a single instrument. See [[MarketData Object]] for a detailed breakdown.
...
| `instrumentType` | string | The [[Market Data Instrument Types|instrument type]]: `Equity`, `Equity Option`, `Future`, `Future Option`, `Cryptocurrency`, `Index`, `Bond`, and others |
| `updatedAt` | datetime | When this market data was last updated |
| `instrument` | [[Instrument Object|Instrument]] | Nested instrument details |
...
Nested instrument metadata returned within each [[MarketData Object|`MarketData` object]]. See [[Instrument Object]] for a detailed breakdown.

| Field | Type | Description |
|-------|------|-------------|
| `symbol` | string | The instrument symbol |
| `instrumentType` | string | The [[Market Data Instrument Types|instrument type]] |
...
| `underlyingInstrument` | [[Instrument Object|Instrument]] | The underlying instrument (for derivatives) |
...
-   **Batch quotes:** Fetch up to 100 symbols in a single request by mixing [[Market Data Instrument Types|instrument types]] (e.g., 50 equities + 30 equity options + 20 futures).
...
-   **100 symbol limit:** The combined total across all [[Market Data Instrument Types|instrument type]] parameters cannot exceed 100 symbols per request. See [[API Rate Limits]].
-   **camelCase field names:** Unlike most [[Tastyworks]] API responses that use kebab-case (`net-liquidating-value`), this endpoint returns fields in camelCase (`netLiquidatingValue`). This is because the Market Data service is a Java-based service with different serialization conventions. See [[API Naming Conventions]].
-   **Singular parameter names:** Use `equity` not `equities`, `equity-option` not `equity-options`, `future` not `futures`. This is a common source of errors. See [[API Naming Conventions]].
-   **Snapshot, not streaming:** This endpoint returns a point-in-time snapshot. For real-time continuous data, use the [[DXLink]] [[WebSocket API|streaming API]] via the quote token from `GET /api-quote-tokens`.
